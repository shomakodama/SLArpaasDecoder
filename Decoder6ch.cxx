...
const int lengths = 1250; // number of data points for each event
...
int Decoder6ch(){
    ...
    if(!fs){
        ...
        while (getline(fs, tmp)) {
            ...
            if(Data == 10){ // header!
                ...
                if(event != 0){ // new event!
                    ...
                    if(plotevent == 1){
                        ...
                        TGraph *gr2 = new TGraph(lengths, TDC_ns, ADC2); // waveform for ch0
                        TGraph *gr3 = new TGraph(lengths, TDC_ns, ADC3); // waveform for ch1
                        TGraph *gr4 = new TGraph(lengths, TDC_ns, ADC4); // waveform for ch0
                        TGraph *gr5 = new TGraph(lengths, TDC_ns, ADC5); // waveform for ch1
                    ...
                    eventflag = std::max({eventflag0, eventflag1, eventflag2, eventflag3, eventflag4, eventflag5});
            ...
            // eventline = 3, 4, 5 -> 1
            // eventline = 6, 7, 8 -> 2
            if(eventline%3 == 0){ // eventline = 0, 3, 6, ...
                ADC1[eventline/3] = ADC_1;
                ADC0[eventline/3] = ADC_2;
            }else if(eventline%3 == 1){ // eventline = 1, 4, 7, ...
                ADC3[eventline/3] = ADC_1;
                ADC2[eventline/3] = ADC_2;
            }else if(eventline%3 == 2){ // eventline = 2, 5, 8, ...
                ADC5[eventline/3] = ADC_1;
                ADC4[eventline/3] = ADC_2;
            }
